...
import torch
sys.path.append(r"/home/pranavs_chib/GroupNet/data"); from dataloader_nba import NBADataset, seq_collate
...
def draw_result(future,past,mode='pre'):
    ...
    for idx in range(50):
        ...
        traj = trajs[idx]
        traj = traj*94/28
        actor_num = traj.shape[0]           #11
        
        length = traj.shape[1]              #15
        length = length - 5
...
def vis_result(test_loader, args):
    ...
    for data in test_loader:
        ...
        p_traj = np.array(data['past_traj']) * args.traj_scale
        ...
        # file_path = "saved_models/model_with3drop_training"
        ...
        previous_3D = np.reshape(p_traj,(batch,actor_num,args.past_length, 2))
...
def test_model_all(test_loader, args):
    ...
    for data in test_loader:
        ...
        # z = np.reshape(p_traj,(batch*actor_num,args.past_length, 2))
        # z = z[None].repeat(20,axis=0)
        # file_path = "saved_models/model_with3drop_training"
        # np.save(file_path, z)
